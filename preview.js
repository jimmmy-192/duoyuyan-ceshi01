(function () {
  ...
  const render = () => { $('previewCount').textContent=String(changes.length); $('previewRows').innerHTML=changes.map((item,index)=>`<article class="preview-row"><div class="preview-cell"><div class="preview-copy-title"><span class="preview-tag">下线</span><strong>${item.title}</strong></div><code>${item.key}</code></div><div class="preview-cell preview-translations">${translations(item.remark)}</div><div class="preview-cell"><strong>${item.version||'所有版本'}</strong></div><div class="preview-cell"><button data-remove="${index}">移除</button></div></article>`).join('') }
  $('previewOfflineButton').onclick=()=>open('previewOfflineModal'); $('previewBatchButton').onclick=()=>open('previewBatchModal'); $('previewModuleButton').onclick=$('previewModuleButton2').onclick=()=>open('previewModuleModal')
  ...
  $('previewConfirmBatch').onclick=()=>{changes.push({key:'batch_import_key',title:'小红书',version:'notes.1.0.16',remark:'批量上传导入'});render();close('previewBatchModal');notify('批量上传内容已确认')}; $('previewSaveModule').onclick=()=>{close('previewModuleModal');notify('模块设置已保存')}; $('previewDraftButton').onclick=()=>notify('草稿已保存'); $('previewSubmitButton').onclick=()=>notify('已提交翻译需求'); render()
})()
